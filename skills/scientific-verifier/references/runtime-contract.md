...
The personal/local profile follows [local-contract.md](local-contract.md). Claude Code supplies the planner/tool loop.

The host must not place a value the planner requires in a reply it may not receive. Claude Code writes an oversized tool result to a file and returns a preview; the local planner has no file-read tool, so anything past that preview is lost to it. The launcher therefore delivers the pinned instructions, the authorized source path and the current token in the planner's prompt, and `get_verifier_context` returns a header bounded well below the smallest observed inline limit, serving bulk sections on request. This limit is enforced by the host, not configured: the threshold that triggers spilling is not exposed as an environment variable or setting in the supported CLI, so a verifier that depends on a large reply arriving intact has no way to guarantee it. The Python launcher starts that native loop and an internal run-bound MCP server; it does not implement a competing model loop. Fresh restricted subject processes receive an explicit recognized skill invocation. Subscription-token and API-key modes use isolated configuration and separate credential environments. Historical Desktop host limitations below apply only to their original profiles.
