...
description: Verify or demonstrate a submitted skill in Claude Chat. Read local or attached safe skills, extract stated behaviors, run useful example tasks, and produce a traceable report. Supports a general same-chat demo and separately configured scientific evaluation profiles.
---
...
**Version 0.5.0 defaults to `profile: demo`.** For any safe skill, use `start_verifier_run` with the local path explicitly supplied by the user, or `start_inline_demo_run` with the attached/pasted skill text and a descriptive source name. Then follow [the demo contract](references/demo-contract.md): load/read the submission, extract its stated behaviors (scientific or otherwise), commit a few useful test cases, generate actual example outputs using this Chat, record each observation, and write the report. Do not wait for a chemistry catalog, external runner, API key, scientific grade or user approval of routine steps. If an external capability cannot run here, record that case as not tested. Explain that outputs and qualitative review came from this same Chat; this is not independent scientific verification.
