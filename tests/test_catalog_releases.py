"""Release preparation, immutable inventory and retirement, using synthetic records only."""
...
class ReleaseTests(unittest.TestCase):
    def setUp(self):
        ...
        self.assessment={"candidate_ref":self.key,"proposal":"propose_for_catalog","prepared_by":"verifier_agent",
                         ...
                         "redistribution":REDISTRIBUTION,"scientific_approval":"not_conferred"}

    def release(self,**kwargs):
        return build(self.store,self.raw,self.settings,self.metadata,[self.assessment],**kwargs)

    def test_preparation_needs_one_complete_unapproved_assessment_per_candidate(self):
        for assessments in ([],[self.assessment,self.assessment],
                            [{**self.assessment,"scientific_approval":"granted"}],
                            [{**self.assessment,"proposal":"approved_for_catalog"}],
                            [{**self.assessment,"candidate_ref":[]}]):
            with self.subTest(assessments=len(assessments)),self.assertRaises(Fault):
                build(self.store,self.raw,self.settings,self.metadata,assessments)
    ...
    def test_changed_bundle_and_incompatible_runtime_fail(self):
        ...
        release["bundle"]["redistribution"]="Changed assessment after preparation."
    ...
    def test_update_requires_increasing_version_and_retains_previous_inventory(self):
        ...
        self.assessment["proposal"]="propose_retirement"
