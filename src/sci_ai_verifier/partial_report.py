...
def write_partial_report(store,run_id,code,reason=None):
    ...
    document={"run_id":run_id,"verification_complete":False,"error_code":code,"reason":reason,"claims":claims,
    ...
    lines=["# Incomplete verification","","Run: "+run_id,"","Stopped: "+escape(code),""]
    if reason:
        lines += ["Reason: "+escape(reason),""]
    lines += [document["note"],""]
    for claim in claims:
