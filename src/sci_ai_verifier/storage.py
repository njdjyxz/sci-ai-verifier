...
SUPPORTED_IMPLEMENTATION_VERSIONS = frozenset({"0.2.0", "0.3.0", "0.4.0", "0.5.0"})
...
def compatible(state):
    ...
    permitted = {"stage2": {"created", "source_ready", "stage2_complete", "incomplete"},
    ...
    if state["profile"] in {"stage3", "verification"}:
        if (writer not in {"0.3.0", "0.4.0", "0.5.0"} or not {"catalog_ref", "routing_ref", "intended_grade"} <= set(state)
    ...
    if state["profile"] == "verification":
        ...
        if writer not in {"0.4.0", "0.5.0"} or not required <= set(state):
    ...
    if state["profile"] == "demo":
        if (writer != "0.5.0" or not {"demo_plan_ref", "demo_observation_refs", "report_ref", "report_markdown_ref", "submission_origin"} <= set(state)
